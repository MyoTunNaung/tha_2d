...
<div class="card">
    ...
    <div class="card-header">
      ...
      {{ csrf_field() }}  
        ...
        <input type="text" name="action"          value="{{ $action }}"         id="action"       >
        ...
        <div role="tabpanel" class="tab-pane"         id="three">
              ...
              <!-- <select name="three_work_file_id" id="three_work_file_id" class="form-control">
                ...
                <br><br><br>
                 ...
                 <select name="current_work_file_id" id="current_work_file_id" class="form-control" > 

                    @foreach($work_files as $work_file)
                      <option value="{{ $work_file->id }}" @if($work_file->id == $current_work_file_id) selected @endif>
                          {{ $work_file->show }}
                      </option>                  
...
<div class="col-md-6 col-md-offset-2">
      ...
      <div class="card" >
                  ...
                  <div class="d-flex justify-content-between align-items-center">
                      ...
                      <select name="slip"  id="slip">
                          <option value="Last_Call"  @if($slip == "Last_Call") selected @endif>Last_Call</option>
                          <option value="All_Call" @if($slip == "All_Call") selected @endif>All_Call</option>
                      </select>
                      &nbsp; Slip: {{ $slip_id }}
                      </span>
                  ...
                  <tbody style="font-size: 16px;font-weight: bold;" id="show_table">
                      ...
                      @if($twosale->slip_id == $slip->slip_id)
                      <tr>
                          ...
                          @if($twosale->type != $col_type)
                          <td> 
                            <a  href="{{ url("/2dsale/typedel/{$twosale->id}?action={$action}&in_out={$in_out}") }}" class="btn btn-danger btn-sm" >TypeDel</a>
                          </td>
                          @else
                          <td></td>
                          @endif
                          ...
                          <td> 
                            <a  href="{{ url("/2dsale/del/{$twosale->id}?action={$action}&in_out={$in_out}") }}" class="btn btn-danger btn-sm" >Del</a>
                          </td>
                      ...
                      @endforeach
                        ...
                        <tr style="background-color: lightgray;font-size: 16px;">
                          <td>Call</td>
                          <td style="text-align: right;">{{ number_format($slip->call_total) }} </td>
                          <td colspan="4"></td>
                        </tr>   
                      ...
                      <tr style="border: 1px solid black;">
                          ...
                          <td> 
                            <a href="{{ url("/2dsale/alldel/{$work_file_id}/{$user_id}/{$customer_id}/{$slip_id}?action={$action}&in_out={$in_out}") }}" class="btn btn-danger btn-sm" >AllDel</a>
                          </td>
...
<div class="card">
    ...
    <form method="post" enctype="multipart/form-data" action="{{ url("/2dsale/add/{$work_file_id}/{$slip_id}") }}">
...
<div class="card">
    ...
    <div class="text-center">
        <span class="card-text text-center">                
          <select name="max_minus"  id="max_minus">
    ...
    <div class="text-center" style="color: red;">Total : {{ number_format($max_total) }}</div>
...
<div class="col-md-6 col-md-offset-2">
  ...
  <div class="card">
          ...
          <div class="d-flex justify-content-between align-items-center">
              ...
              <span class="card-text">
                    <a  class="btn btn-secondary btn-sm" href="{{ url("/2dsale/image-text/bet/{$work_file_id}?action={$action}&in_out={$in_out}") }}" id="image-text">
                     Image/Text Bet
                    </a>
               </span>

              <span class="card-text">
                    <a  class="btn btn-secondary btn-sm" href="{{ url("/2dsale/padaythar/bet/{$work_file_id}?action={$action}&in_out={$in_out}") }}" id="padaythar_bet">
                      Padaythar Bet
                    </a>
              </span>  
 ...
 -->
           ...
           <table class="table1 table-stripped table-sm" style="font-size: 12px;">
                ...
                <tr>                 
                    ...
                    <td colspan="2" >
                      <input type="button" class="btn btn-info btn-sm" id="OK" value="OK" onclick = "enter()" style="width: 100%;font-size: 18px;">
                    </td>
...
$(document).ready(function()
{
    ...
    $("#customer_id").change(function()
    {
      ...
      if(customer_id)
      {
        $.ajax(
            {
                type:   "GET",
                url:  "{{ url('saveCustomer') }}?user_id="+user_id
                                              +"&customer_id="+customer_id,
    ...
    {    
      ...
      if(user_id)
      {
        $.ajax(
            {
                type:   "GET",
                url:  "{{ url('saveUser') }}?user_id="+user_id
                                              +"&customer_id="+customer_id,
    ...
    // 2D AM WorkFile Change
    $("#two_am_work_file_id").change(function()
    {    

      var two_am_work_file_id       = $('#two_am_work_file_id').val(); 
              
      if(two_am_work_file_id)
      {
        $.ajax(
            {
                type:   "GET",
                url:  "{{ url('saveTwoAMWorkFile') }}?two_am_work_file_id="+two_am_work_file_id,
                dataType: 'json',             
                success:function(res)
                {                                                   
    ...
    // End 2D AM WorkFile Change

    // 2D AM WorkFile Change
    $("#two_pm_work_file_id").change(function()
    {    

      var two_pm_work_file_id       = $('#two_pm_work_file_id').val(); 
              
      if(two_pm_work_file_id)
      {
        $.ajax(
            {
                type:   "GET",
                url:  "{{ url('saveTwoPMWorkFile') }}?two_pm_work_file_id="+two_pm_work_file_id,
                dataType: 'json',             
                success:function(res)
                {                                                   
    ...
    {    
      ...
      var three_work_file_id       = $('#three_work_file_id').val(); 
              
      if(three_work_file_id)
      {
        $.ajax(
            {
                type:   "GET",
                url:  "{{ url('saveThreeWorkFile') }}?three_work_file_id="+three_work_file_id,
                dataType: 'json',             
                success:function(res)
                {                                                   
  ...
  $(document).ready(function()
{
    ...
    //2D AM Panel Click => get WorkFile
     $("#two_am_panel").click(function()
    {
        var name      = "2D";
        var duration  = "AM";
          ...
          $.ajax(
              {
                  ...
                  {                                                   
                        ...
                        for(var count = 0; count < res.length; count++)
                        {
                            ...
                            html += '<option  value="'+res[count].id+'" >'+ res[count].show +'</option>';                                                 
                          
                        }

                        $("#current_work_file_id").html(html);                        
                        $('#current_work_file_id').change();
                  }
              } 
             );       
        }
     ...
     //2D PM Panel Click => get WorkFile
     $("#two_pm_panel").click(function()
    {
        var name      = "2D";
        var duration  = "PM";
     ...
     //3D PM Panel Click => get WorkFile
     $("#three_panel").click(function()
    {
        var name      = "3D";
        var duration  = "PM";
     ...
     //3D PM Panel Click => get WorkFile
      ...
      //WorkFile Change => Save to Choice
    $("#current_work_file_id").change(function()
    {    

      var current_work_file_id       = $('#current_work_file_id').val(); 
              
      if(current_work_file_id)
      {
        $.ajax(
            {
                type:   "GET",
                url:  "{{ url('saveWorkFile') }}?current_work_file_id="+current_work_file_id,
                dataType: 'json',             
                success:function(res)
                {                                                   
    ...
    // End WorkFile Change => Save to Choice
